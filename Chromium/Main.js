...
let state = {
  ...
  isMiniScreen: false,
  miniScreenPosition: { x: null, y: null },
  dragStartPosition: { x: null, y: null },
...
/**
 * Show episodes list panel with smooth animation
 */
async function showEpisodesList() {
  ...
  try {
    ...
    if (existingPanel) {
      ...
      setTimeout(() => existingPanel.remove(), 300); // Wait for animation
      return;
    }
    ...
    // Add loading animation while content loads
    panel.innerHTML = `
    ...
    // Build the content with season groups
    const content = `
    ...
    // Add click event listeners for episodes
    ...
    showMessage("Failed to load episodes list");
  }
}
